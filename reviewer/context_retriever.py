...
class ContextRetriever:
    ...
    def get_symbol_context(self, git_diff):
        ...
        for symbol in symbols:
            symbol_context = self.code_indexer.retrieve_symbol_context(symbol)
            context.extend(symbol_context)  # 展开列表而不是添加嵌套列表
        return context
    ...
    def get_diff_context(self, git_diff):
        ...
        try:
            ...
            print(f"Error in get_diff_context: {e}")
            return []
    ...
    def dedup_context(self, context_docs):
        ...
        for doc in context_docs:
            ...
            if not isinstance(doc, dict):
                continue
            # 确保必要的键存在
            if 'file_path' not in doc or 'start_line' not in doc or 'end_line' not in doc:
                continue
