...
class HTMLFormatter(CodeReviewFormatter):
    ...
    def _get_severity_css_class(self, severity: Optional[SeverityLevel]) -> str:
        """获取严重程度对应的CSS类"""
        if not severity:
        ...
        return class_map.get(severity, "severity-medium")
                ...
                file_comments = [
                    {
                        ...
                        "severity": comment.severity.value if comment.severity else "Medium",
                        "severity_class": self._get_severity_css_class(comment.severity)
                    }
